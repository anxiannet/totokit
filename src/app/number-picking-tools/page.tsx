...
export default function NumberPickingToolsListPage() {
  ...
  const { data: predictionsForTargetDrawMap = {}, isLoading: isLoadingPredictions } = useQuery<Record<string, number[]>, Error>({
    queryKey: ["predictionsForDrawListPage", OFFICIAL_PREDICTIONS_DRAW_ID],
    ...
    <div className="container mx-auto px-4 py-8 md:px-6 md:py-12">
      ...
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            探索多种选号策略。下方卡片直接展示各工具为第 {OFFICIAL_PREDICTIONS_DRAW_ID} 期生成的预测号码 (由管理员保存)。点击“查看详情与历史表现”可深入分析其算法在过去10期开奖中的动态预测情况。
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingPredictions && (
            <div className="flex justify-center items-center min-h-[200px]">
              ...
              <p className="ml-3 text-muted-foreground">正在加载第 {OFFICIAL_PREDICTIONS_DRAW_ID} 期预测号码...</p>
            </div>
          )}
                    ...
                    <CardContent className="flex-grow">
                      <div className="mb-3">
                        <p className="text-xs font-medium text-muted-foreground mb-1 flex items-center">
                          ...
                          第 {OFFICIAL_PREDICTIONS_DRAW_ID} 期预测 ({currentPrediction.length} 个):
                        </p>
                        {currentPrediction.length > 0 ? (
                          ...
                          <p className="text-xs text-muted-foreground italic">此工具对第 {OFFICIAL_PREDICTIONS_DRAW_ID} 期的预测尚未生成。</p>
                        )}
                      </div>
                    </CardContent>
                    <CardFooter>
