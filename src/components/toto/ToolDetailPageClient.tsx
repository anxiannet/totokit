...
import { ArrowLeft, Target, Loader2, AlertCircle, DatabaseZap, TrendingUp, TrendingDown, Info, BarChartBig, Wand2, PlayCircle, ListOrdered, Save } from "lucide-react";
...
export function ToolDetailPageClient({
  ...
  // State for numbers generated by admin for the current official draw before saving
  ...
  const [isSavingCurrentDraw, setIsSavingCurrentDraw] = useState(false); // Kept for specific save feedback
  const [isSavingHistorical, setIsSavingHistorical] = useState(false); // Kept for specific save feedback

  const isAdmin = user && user.email === "admin@totokit.com";

  const handleGenerateAndSaveAllPredictions = async () => {
    if (!serializableTool || !allHistoricalDataForPerformanceAnalysis || !user?.uid) {
      toast({ title: "错误", description: "无法生成和保存预测，缺少必要信息或权限。", variant: "destructive" });
      return;
    }
    setIsLoadingAllPredictions(true);
    setCurrentPredictionForSave(null);
    setDisplayedHistoricalPerformances(null); // Clear previous displays
    ...
    try {
      ...
      } else if (allHistoricalDataForPerformanceAnalysis.length >= 11) {
        toast({ title: "无历史表现数据", description: `工具 ${serializableTool.name} 的历史表现无法计算或无结果。`, variant: "default" });
      ...
      if (currentPredResult && currentPredResult.length > 0) {
        ...
        try {
          ...
          if (saveCurrentResult.success) {
            ...
            setSavedPredictionForTargetDraw(currentPredResult); // Reflect saved state
      ...
      if (historicalPerfResult && historicalPerfResult.length > 0) {
        ...
        const predictionsToSaveForDb: Array<Omit<ToolPredictionInput, 'toolId' | 'toolName' | 'userId'>> = historicalPerfResult.map(hp => ({
      ...
      if(currentSaveSuccess || historicalSaveSuccess){
        ...
        // No data was generated to begin with
      } else {
        ...
        <CardContent>
          ...
          {isAdmin && (
            <div className="mb-6 pb-4 border-b">
              <Button onClick={handleGenerateAndSaveAllPredictions} disabled={isLoadingAllPredictions || isSavingCurrentDraw || isSavingHistorical} className="w-full">
              ...
              {(isLoadingAllPredictions || isSavingCurrentDraw || isSavingHistorical) && (
                <p className="text-center text-sm text-muted-foreground mt-2">
                  {isLoadingAllPredictions ? "正在生成预测数据..." : (isSavingCurrentDraw || isSavingHistorical) ? "正在保存预测数据..." : ""} 请稍候...
                </p>
              )}
            </div>
          )}
          ...
          <div className="mb-6 pb-6 border-b">
            ...
            {savedPredictionForTargetDraw && savedPredictionForTargetDraw.length > 0 ? (
              ...
              <>
                <p className="text-xs text-muted-foreground mb-1">新生成的预测 (等待管理员操作保存):</p>
              ...
              <Alert variant="default" className="mt-3">
                ...
                <AlertDescription>
                  期号 {OFFICIAL_PREDICTIONS_DRAW_ID} 的预测号码尚未由管理员生成{isAdmin ? "" : "或保存"}。
                  {isAdmin && " 请点击上方的“开始全面预测并保存”按钮生成并保存。"}
                </AlertDescription>
              </Alert>
            )}
          </div>
          ...
          {(displayedHistoricalPerformances !== null || isLoadingAllPredictions) && (
             <div className="pt-0">
