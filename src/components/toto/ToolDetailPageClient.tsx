...
export function ToolDetailPageClient({
  ...
  // Initial fetch for official prediction
  useEffect(() => {
      ...
      <Card className="w-full">
        ...
        <CardContent>
          ...
          <div className="pt-0">
            ...
            {!displayedHistoricalPerformances && !isLoadingHistoricalPerformance && (
            ...
            {isLoadingHistoricalPerformance && (
                <div className="flex items-center justify-center h-40">
                    ...
                    <p className="ml-2 text-muted-foreground">正在计算历史表现...</p>
                </div>
            )}
            {displayedHistoricalPerformances && displayedHistoricalPerformances.length > 0 && (
              <>
                <ScrollArea className="h-[calc(100vh-600px)] rounded-md border p-3 space-y-4 bg-background/50">
                 ...
                 {isAdmin && allHistoricalDataForPerformanceAnalysis && allHistoricalDataForPerformanceAnalysis.length > 0 && (
                  <div className="mt-6 pb-6 border-t pt-4">
                    <h4 className="text-md font-semibold mb-2 flex items-center gap-1.5">
                      <DatabaseZap className="h-5 w-5 text-blue-600" />
                      管理员操作: 历史回测数据
                    </h4>
                    <Button onClick={handleSaveHistoricalBacktests} disabled={isSavingHistoricalBacktests || !user?.uid || !displayedHistoricalPerformances || displayedHistoricalPerformances.length === 0} className="w-full">
